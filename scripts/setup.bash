...
if [ ! -d "$OPENCV_DIR_BUILD" ]; then
    ...
    if [ "$ARCH" = "arm64" ]; then
		cmake -DBUILD_SHARED_LIBS="OFF" -DCMAKE_INSTALL_PREFIX="$OPENCV_DIR_INSTALL" -DCMAKE_CXX_FLAGS="$OPENCV_CFLAGS" -DCMAKE_C_FLAGS="$OPENCV_CFLAGS" -DCMAKE_TOOLCHAIN_FILE="$OPENCV_DIR_SRC/platforms/linux/aarch64-gnu.toolchain.cmake" -DWITH_CUDA="0" -DWITH_LAPACK="0" -DENABLE_PRECOMPILED_HEADERS=OFF "$OPENCV_DIR_SRC"
	else
