...
DepthGroundRemover::RepairDepth(const Mat& depth_image)
{
	...
	cv::filter2D(depth_image, inpainted_depth, SAME_OUTPUT_TYPE, kernel, ANCHOR_CENTER, 0,
...
Mat
DepthGroundRemover::GetSavitskyGolayKernel(int window_size) const
{
	if (window_size % 2 == 0)
	{
		throw std::logic_error("only odd window size allowed");
	}
	bool window_size_ok = window_size == 5 || window_size == 7 || window_size == 9
			|| window_size == 11;
	if (!window_size_ok)
	{
...
Mat
DepthGroundRemover::GetUniformKernel(int window_size, int type) const
{
	if (window_size % 2 == 0)
	{
		throw std::logic_error("only odd window size allowed");
...
DepthGroundRemover::ApplySavitskyGolaySmoothing(const Mat& image, int window_size)
{
	...
	cv::filter2D(image, smoothed_image, SAME_OUTPUT_TYPE, kernel, ANCHOR_CENTER, 0,
			cv::BORDER_REFLECT101);
